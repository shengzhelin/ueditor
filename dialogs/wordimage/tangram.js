...
/**
 * 對語言層面的封裝，包括類型判斷、模塊擴展、繼承基類以及對象自定義事件的支持。
 ...
 * 在Browser identification最後添加Version + 數字進行版本標識
 * opera後面的數字保持在9.80不變
 */
...
/**
 * 操作flash對象的方法，包括創建flash對象、獲取flash對象以及判斷flash插件的版本號
 * @namespace baidu.swf
 */
baidu.swf = baidu.swf || {};
...
baidu.flash._Base = (function(){
    ...
    return function(options, callBack){
         ...
         * @param {Array} params 傳入的參數組成的數組,若不許要參數，需傳入空數組
         * @param {Function} [callBack] 異步調用後將返回值作為參數的調用回調函數，如無返回值，可以不傳入此參數
         * @return {Null}
        */
 ...
 * @config {String} vars.uploadDataFieldName POST請求中圖片數據的key,默認值'picdata'
