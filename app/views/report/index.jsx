...
const columns=(page,handler={})=>[
  ...
  },
  {
    ...
    ellipsis: true,
    align:'center',
    render:(text,row,index)=>text=='0'?<Tag color="green">啟用</Tag>:<Tag color="red">未啟用</Tag>,
  },
  {
    title: '監控狀態',
    dataIndex: 'status',
      ...
      return (
        <div className="table-actions">
          <a onClick={() => handleItemEdit(row)} className="edit">
            <Tooltip title="編輯"><EditOutlined /></Tooltip>
          </a>
          <a onClick={() => handleItemDel(row)} className="delete">
            <Tooltip title="刪除"><DeleteOutlined /></Tooltip>
          </a>
        </div>
      );
    },
...
const Index=props=>{
  ...
  return <div className="host-list-page">
    <div className="search-bar">
      <Select value={selecteds?.group} allowClear placeholder="請選擇主機組" style={{ width: 150,marginRight: 10 }} onChange={handleGroup}>
        {
          (items||[]).map(v=><Option key={v.groupid} value={v.groupid}><Tooltip title={v.name}>{v.name}</Tooltip></Option>)
        }
      </Select>
      <Select value={selecteds?.host} allowClear placeholder="請選擇主機" style={{ width: 120,marginRight: 10 }} onChange={handleHost}>
        {
          (hosts||[]).map(v=><Option key={v.hostid} value={v.hostid}><Tooltip title={v.name}>{v.name}</Tooltip></Option>)
        }
      </Select>
      <Select value={selecteds?.item} allowClear placeholder="請選擇指標" style={{ width: 180,marginRight: 10 }} onChange={handleTarget}>
        {
          (targets||[]).map(v=><Option key={v.itemid} value={v.itemid}><Tooltip title={v.name}>{v.name}</Tooltip></Option>)
        }
      </Select>
      <RangePicker showTime placeholder={['請選擇開始時間','請選擇結束時間']} value={rt} onChange={(moment,str)=>setSearchValue({begin:str[0],end:str[1]})} style={{marginRight:'15px'}} />
      <Button type="primary" loading={detailLoading} onClick={()=>dlDetail()} icon={<SearchOutlined />} style={{ marginRight: 10 }}>導出詳情</Button>
      <Button type="primary" loading={trendLoading} onClick={()=>dlTrend()} icon={<SearchOutlined />}>導出趨勢</Button>
    </div>
