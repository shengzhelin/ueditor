...
export function rowColumnOperationInitial(){
    ...
    $("#luckysheet-cols-h-c").mousedown(function (event) {
        ...
        Store.orderbyindex = col_index;//排序全局函數
...
function luckysheetcolsdbclick() {
    ...
    for(let s = 0; s < Store.luckysheet_select_save.length; s++){
        ...
        if (colIndex < c1 || colIndex > c2) {
            if(colIndex in matchColumn){//此列已計算過
                continue;
            }
            ...
            for(let r = dataset_row_st; r <= dataset_row_ed; r++){
                ...
                // let value = getcellvalue(r, colIndex, d, "m").toString(); //單元格文本
                // let textMetrics = getMeasureText(value, canvas).width; //文本寬度
            ...
            for (let c = c1; c <= c2; c++) {
                ...
                for(let r = dataset_row_st; r <= dataset_row_ed; r++){
                    ...
                    // let value = getcellvalue(r, c, d, "m").toString(); //單元格文本
                    // let textMetrics = getMeasureText(value, canvas).width; //文本寬度
