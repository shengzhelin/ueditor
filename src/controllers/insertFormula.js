...
const insertFormula = {
    init: function(){
        ...
        $(document).off("keyup.fxSFLI").on("keyup.fxSFLI", "#searchFormulaListInput", function(){
            ...
            $("#formulaTypeList .listBox:first-child").addClass("on"); //默認公式列表第一個為選中狀態
        });
    ...
    formulaListDialog: function(){
        ...
        _this.formulaListByType("0"); //默認公式列表為類型0
    ...
    formulaParmDialog: function(formulaTxt, parm){ //參數彈出框
        let parm_title = '',
            parm_content = '',
            parm_list_content = '';
        ...
        $("#luckysheet-search-formula-parm .parmBox").each(function(index,e){
            ...
            if(formula.getfunctionParam(parmtxt).fn == null){ //參數不是公式
                if(formula.iscelldata(parmtxt)){ //參數是選區
                    let txtdata = luckysheet_getcelldata(parmtxt).data;

                    if(getObjType(txtdata) == "array"){ //參數為多個單元格選區
                        let txtArr = [];
