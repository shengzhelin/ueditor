...
const dataVerificationCtrl = {
    ...
    init: function(){
        ...
        $(document).off("click.dvRangeConfirm").on("click.dvRangeConfirm", "#luckysheet-dataVerificationRange-dialog-confirm", function(e) {
            ...
            if(_this.getRangeByTxt(txt).length > 1){
                tooltip.info('<i class="fa fa-exclamation-triangle"></i>', '不能對多重選擇區域執行此操作，請選擇單個區域，然後再試');
                return;
            }
    ...
    getHintText: function(item) {
        ...
        if(hintText.length == 0){
            if(Store.lang == 'en'){
                ...
                if(item.type == 'dropdown'){
                    hintText += '請選擇下拉列表中的選項';
                }
                else if(item.type == 'checkbox'){

                }
                else if(item.type == 'number' || item.type == 'number_integer' || item.type == 'number_decimal'){
                    hintText += '請輸入' + _this.optionLabel[item.type2] + item.value1;

                    if(item.type2 == 'bw' || item.type2 == 'nb'){
                        hintText += '和' + item.value2 + '之間';
                    }

                    hintText += '的' + _this.optionLabel[item.type];
                }
                else if(item.type == 'text_content'){
                    hintText += '請輸入內容' + _this.optionLabel[item.type2] + item.value1 + '的文本';
                }
                else if(item.type == 'text_length'){
                    hintText += '請輸入長度' + _this.optionLabel[item.type2] + item.value1;
                    ...
                    hintText += '請輸入正確的' + _this.optionLabel[item.type2];
                }
            }
        }
    ...
    getFailureText: function(item) {
        ...
        if(Store.lang == 'en'){
            ...
            if(item.type == 'dropdown'){
                failureText += '你選擇的不是下拉列表中的選項';
            }
            else if(item.type == 'checkbox'){

            }
            else if(item.type == 'number' || item.type == 'number_integer' || item.type == 'number_decimal'){
                failureText += '你輸入的不是' + _this.optionLabel[item.type2] + item.value1;

                if(item.type2 == 'bw' || item.type2 == 'nb'){
                    failureText += '和' + item.value2 + '之間';
                }

                failureText += '的' + _this.optionLabel[item.type];
            }
            else if(item.type == 'text_content'){
                failureText += '你輸入的不是內容' + _this.optionLabel[item.type2] + item.value1 + '的文本';
            }
            else if(item.type == 'text_length'){
                failureText += '你輸入的不是長度' + _this.optionLabel[item.type2] + item.value1;
                ...
                failureText += '你輸入的不是一個正確的' + _this.optionLabel[item.type2];
            }
        }
...
function validateIdCard(idCard) {
    ...
    if (regIdCard.test(idCard)) {
        if (idCard.length == 18) {
            ...
            var idCardY = new Array(1, 0, 10, 9, 8, 7, 6, 5, 4, 3, 2); // 這是除以11後，可能產生的11位余數、驗證碼，也保存成數組
