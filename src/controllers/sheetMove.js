    ...
    if(direction == "down"){
        ...
        for(let r = strIndex + 1; r <= endIndex; r++){
            ...
            if(cellNull.length == 1 && stNull==true && cellNull[i] == false){
                ...
                if(stNull && cellNull[i] == false){ //起始是空，找第一個有值的位置
                    index = strIndex + i + 1;
                    break;
                }
                else if(!stNull){ //起始有值，找一個有值的位置
                    if(cellNull[i] == false && cellNull[i - 1] == true){ //前面為空
                        index = strIndex + i + 1;
                        break;
                    }
                    else if(cellNull[i] == true && cellNull[i - 1] == false){ //後面為空
                        index = strIndex + i;
                        break;
                    }
                }
