...
const luckysheetFreezen = {
    ...
    // 定義凍結首行、首列是實際的第一行第一列還是當前視圖的第一行第一列
    // excel 為視圖的第一行第一列，但此處實現有問題，如滾動到15行凍結首行，當前凍結了15行，保存再進去實際凍結了第一行
    // 凍結真實的第一行、第一列更符合直覺
    ...
    scrollAdaptOfselect: function(){
        ...
        if (_this.freezenhorizontaldata != null && _this.freezenverticaldata != null) {
            ...
            for(let s = 0; s < Store.luckysheet_select_save.length; s++){
                ...
                if(r1 >= freezen_rowindex){//原選區在凍結區外
                ...
                else if(r2 >= freezen_rowindex){//原選區有一部分在凍結區內
                    if(top_move + height_move < freezenTop + offTop){
                        $("#luckysheet-cell-selected-boxs").find(".luckysheet-cell-selected").eq(s).show().css({
                            "top": top_move + offTop,
                ...
                if(c1 >= freezen_colindex){//原選區在凍結區外
                ...
                else if(c2 >= freezen_colindex){//原選區有一部分在凍結區內
                    if(left_move + width_move < freezenLeft + offLeft){
                        $("#luckysheet-cell-selected-boxs").find(".luckysheet-cell-selected").eq(s).show().css({
     ...
     * @param {Number | String} order 工作表索引；默認值為當前工作表索引
     * @param {Object} focus 凍結選區時的focus單元格行列值構成的對象；格式為{ row_focus:0, column_focus:0 }
     */
