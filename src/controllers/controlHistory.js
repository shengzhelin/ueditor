...
const controlHistory = {
    redo: function (e) {
        ...
        if (ctr.type == "datachange") {
           ...
           // jfrefreshgrid(ctr.data, ctr.range, allParam);
            ...
            /* ⚠️  這個🌶️  dataRange表示的才是數據更新的位置 */
            jfrefreshgrid(ctr.data, ctr.dataRange, allParam);

            // formula.execFunctionGroup(null, null, null, null, ctr.data);//取之前的數據
        }
        ...
        else if (ctr.type == "addRC") { //增加行列撤銷操作
        ...
        else if (ctr.type == "delRC") { //刪除行列撤銷操作
            let ctrlValue = $.extend(true, {}, ctr.ctrlValue);
        ...
        else if (ctr.type == "deleteCell") { //刪除單元格撤銷操作
            jfrefreshgrid_deleteCell(
                ctr.data, 
                ctr.config, 
                ctr.ctrl, 
        ...
        else if (ctr.type == "showHidRows") { // 隱藏、顯示行 撤銷操作
            //config
            Store.config = ctr.config;
        ...
        else if (ctr.type == "showHidCols") { // 隱藏、顯示列 撤銷操作
            //config
            Store.config = ctr.config;
            Store.luckysheetfile[getSheetIndex(ctr.sheetIndex)].config = ctr.config;
    ...
    undo: function () {
        ...
        if (ctr.type == "datachange") {
        ...
        else if (ctr.type == "delRC") { //刪除行列重做操作
            jfrefreshgrid_adRC(
                ctr.curData, 
                ctr.curConfig, 
                "delRC", 
        ...
        else if (ctr.type == "deleteCell") { //刪除單元格重做操作
            jfrefreshgrid_deleteCell(
                ctr.curData, 
                ctr.curConfig, 
                ctr.ctrl, 
        ...
        else if (ctr.type == "showHidRows") { // 隱藏、顯示行 重做操作
            //config
            Store.config = ctr.curconfig;
        ...
        else if (ctr.type == "showHidCols") { // 隱藏、顯示列 重做操作
            //config
            Store.config = ctr.curconfig;
            Store.luckysheetfile[getSheetIndex(ctr.sheetIndex)].config = ctr.curconfig;
            ...
            $("#luckysheet-filter-selected span").data("type", "0").data("type", null).text("無");
