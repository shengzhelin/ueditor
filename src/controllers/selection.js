...
const selection = {
    ...
    copy: function (e) {//copy事件
        ...
        //copy範圍數據拼接成table 賦給剪貼板
        let _this = this;

        let borderInfoCompute;
        if(Store.config["borderInfo"] && Store.config["borderInfo"].length > 0){ //邊框
    ...
    pasteHandler: function (data, borderInfo) {
        ...
        if(Store.luckysheet_select_save.length > 1){
            if(isEditMode()){
                alert("不能對多重選擇區域執行此操作，請選擇單個區域，然後再試");
            }
            else{
                tooltip.info('<i class="fa fa-exclamation-triangle"></i>提示', "不能對多重選擇區域執行此操作，請選擇單個區域，然後再試");
            }
        }
        ...
        if (typeof data == "object") {
            ...
            let minh = Store.luckysheet_select_save[0].row[0], //應用範圍首尾行
                maxh = minh + copyh - 1;
            ...
            let d = editor.deepCopyFlowData(Store.flowdata);//取數據
            ...
            for (let r = 0; r < rlen; r++) {
                ...
                for (let c = 0; c < clen; c++) {
                    ...
                    if(isRealNum(value)){
                        // 如果單元格設置了純文本格式，那麽就不要轉成數值類型了，防止數值過大自動轉成科學計數法
    ...
    pasteHandlerOfCopyPaste: function(copyRange){
        ...
        //應用範圍
        let last = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1];
        let minh = last["row"][0], maxh = last["row"][1];         //應用範圍首尾行
        let minc = last["column"][0], maxc = last["column"][1];   //應用範圍首尾列

        let mh = (maxh - minh + 1) % copyh;
        let mc = (maxc - minc + 1) % copyc;

        if(mh != 0 || mc != 0){ //若應用範圍不是copydata行列數的整數倍，則取copydata的行列數
            maxh = minh + copyh - 1;
            maxc = minc + copyc - 1;
        }
    ...
    pasteHandlerOfPaintModel: function(copyRange){
        ...
        let timesH = Math.ceil((maxh - minh + 1) / copyh);  //覆制行 組數
        let timesC = Math.ceil((maxc - minc + 1) / copyc);  //覆制列 組數
