...
function collaborativeEditBox() {
    let all_width = Store.visibledatacolumn;//當前操作頁的所有列距離左邊的距離
    let all_height = Store.visibledatarow;//當前操作頁的所有列距離頂部的距離

    Store.cooperativeEdit.changeCollaborationSize.forEach(value => {
        if (value.i == Store.currentSheetIndex) {
            let count_col = value.v.column;//系統提示框所在的列範圍
            let change_width = all_width[count_col[0]] -1 //提示框所在列號為0時要改變的寬
            if(value.v.column[0] !== 0)  {
                //用提示框右邊框到圖表最左的距離減去左邊框到圖表左邊距離再減去邊框值
                change_width = all_width[count_col[1]] - all_width[count_col[0] - 1] - (count_col[1] - count_col[0] + 1)
            }
            let count_row = value.v.row;//系統提示框所在的行範圍
            ...
            if (Store.config.columnlen !== null) {
                //當改變寬的列不在提示框範圍內時，將改變列的初始位置改為在提示框範圍內
