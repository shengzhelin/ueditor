...
const conditionformat = {
    ...
    selectStatus: false,
    dataBarList: [
        { "format": ["#638ec6", "#ffffff"] },  //藍-白漸變 數據條
        { "format": ["#63c384", "#ffffff"] },  //綠-白漸變 數據條
        { "format": ["#ff555a", "#ffffff"] },  //紅-白漸變 數據條
        { "format": ["#ffb628", "#ffffff"] },  //橙-白漸變 數據條
        { "format": ["#008aef", "#ffffff"] },  //淺藍-白漸變 數據條
        { "format": ["#d6007b", "#ffffff"] },  //紫-白漸變 數據條

        { "format": ["#638ec6"] },  //藍色 數據條
        { "format": ["#63c384"] },  //綠色 數據條
        { "format": ["#ff555a"] },  //紅色 數據條
        { "format": ["#ffb628"] },  //橙色 數據條
        { "format": ["#008aef"] },  //淺藍色 數據條
        { "format": ["#d6007b"] }   //紫色 數據條
    ],
    colorGradationList: [
        { "format": ["rgb(99, 190, 123)", "rgb(255, 235, 132)", "rgb(248, 105, 107)"] },  //綠-黃-紅色階
        { "format": ["rgb(248, 105, 107)", "rgb(255, 235, 132)", "rgb(99, 190, 123)"] },  //紅-黃-綠色階

        { "format": ["rgb(99, 190, 123)", "rgb(252, 252, 255)", "rgb(248, 105, 107)"] },  //綠-白-紅色階
        { "format": ["rgb(248, 105, 107)", "rgb(252, 252, 255)", "rgb(99, 190, 123)"] },  //紅-白-綠色階

        { "format": ["rgb(90, 138, 198)", "rgb(252, 252, 255)", "rgb(248, 105, 107)"] },  //藍-白-紅色階
        { "format": ["rgb(248, 105, 107)", "rgb(252, 252, 255)", "rgb(90, 138, 198)"] },  //紅-白-藍色階

        { "format": ["rgb(252, 252, 255)", "rgb(248, 105, 107)"] },  //白-紅色階
        { "format": ["rgb(248, 105, 107)", "rgb(252, 252, 255)"] },  //紅-白色階

        { "format": ["rgb(99, 190, 123)", "rgb(252, 252, 255)"] },  //綠-白色階
        { "format": ["rgb(252, 252, 255)", "rgb(99, 190, 123)"] },  //白-綠色階

        { "format": ["rgb(99, 190, 123)", "rgb(255, 235, 132)"] },  //綠-黃色階
        { "format": ["rgb(255, 235, 132)", "rgb(99, 190, 123)"] }   //黃-綠色階
    ],
    init: function(){
        let _this = this;
        ...
        $(document).off("click.CFnewConditionRuleConfirm").on("click.CFnewConditionRuleConfirm", "#luckysheet-newConditionRule-dialog-confirm", function(){
            ...
            if(index == 0){
                if(type1 == "dataBar"){ //數據條
                    ...
                    if(type2 == "gradient"){ //漸變填充
                        format = [color, "#ffffff"];
                    }
                    else if(type2 == "solid"){ //實心填充
                        format = [color];
                    }
                ...
                else if(type1 == "colorGradation"){ //色階
        ...
        $(document).off("click.CFdefault").on("click.CFdefault", "#luckysheet-conditionformat-dialog-confirm", function(){
            ...
            else if(conditionName == "betweenness"){//介於
            ...
            else if(conditionName == "duplicateValue"){//重覆值
            ...
            else if(conditionName == "AboveAverage"){ //高於平均值
                conditionValue.push("AboveAverage");
            }
            else if(conditionName == "SubAverage"){ //低於平均值
                conditionValue.push("SubAverage");
            }
    ...
    getConditionRuleList: function(index){
        ...
        let ruleArr = _this.fileClone[getSheetIndex(index)].luckysheet_conditionformat_save; //條件格式規則集合
        if(ruleArr != null && ruleArr.length > 0){
            const conditionformat_Text = locale().conditionformat;

            for(let i = 0; i < ruleArr.length; i++){
                let type = ruleArr[i]["type"];            //規則類型
                let format = ruleArr[i]["format"];        //規則樣式
                let cellrange = ruleArr[i]["cellrange"];  //規則應用範圍

                let ruleName;         //規則名稱
                let formatHtml = '';  //樣式dom
                if(type == "dataBar"){
    ...
    daterangeInit: function(id){
        ...
        $('#' + id).find("#daterange-btn").flatpickr({
            ...
            onChange: function (data, label) {
                ...
                //label:通過它來知道用戶選擇的是什麽，傳給後台進行相應的展示
                let format1 = [
            ...
            for(let i = 0; i < ruleArr.length; i++){
                ...
                if(type == "dataBar"){ //數據條
                    ...
                    if(max != null && min != null){
                        if(min < 0){ //選區範圍內有負數
                            let plusLen = Math.round(max / (max - min) * 10) / 10;                //正數所占比
                            let minusLen = Math.round(Math.abs(min) / (max - min) * 10) / 10;     //負數所占比
                ...
                else if(type == "colorGradation"){ //色階
                    ...
                    //獲取變量值
                    let conditionName = ruleArr[i].conditionName,         //條件名稱
                        conditionValue0 = ruleArr[i].conditionValue[0],   //條件值1
                        conditionValue1 = ruleArr[i].conditionValue[1],   //條件值2
                        textColor = format.textColor,          //條件格式文本顏色 fc
                        cellColor = format.cellColor;          //條件格式單元格顏色 bg
                        ...
                        if(conditionName == "greaterThan" || conditionName == "lessThan" || conditionName == "equal" || conditionName == "textContains"){
                            ...
                            if(conditionName == "top10" || conditionName == "top10%" || conditionName == "last10" || conditionName == "last10%"){
                                ...
                                if(conditionName == "top10"){
                                    cArr = dArr.slice(0, conditionValue0); //前10項數組
                                }
                                else if(conditionName == "top10%"){
                                    cArr = dArr.slice(0, Math.floor(conditionValue0*dArr.length/100)); //前10%數組
                                }
                                else if(conditionName == "last10"){
                                    cArr = dArr.slice((dArr.length-conditionValue0), dArr.length); //最後10項數組
                                }
                                else if(conditionName == "last10%"){
                                    cArr = dArr.slice((dArr.length-Math.floor(conditionValue0*dArr.length/100)), dArr.length); //最後10%數組
                                }
                                //循環應用範圍計算
                                for(let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r++){
                                    for(let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c++){
                                ...
                                //循環應用範圍計算
                                if(conditionName == "AboveAverage"){ //高於平均值
                                    for(let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r++){
                                        for(let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c++){
                                            if(d[r] == null || d[r][c] == null){
                                                continue;
