...
const server = {
    ...
    historyParam: function(data, sheetIndex, range) {
	    ...
	    if(r1 == r2 && c1 == c2){ //單個單元格更新
	        ...
	        let n = Math.ceil(rowlen / timeR); //分批次更新，一次最多1000個單元格
    ...
    openWebSocket: function(){
        ...
        method.createHookFunction('cooperativeMessage', data)
                ...
                if (sheetToUpdate !== null) {
                  setTimeout(() => {
	            ...
	            else if(type == 3){ //多人操作不同選區("t": "mv")（用不同顏色顯示其他人所操作的選區）
	                let id = data.id;
					...
					let r = 0
					let c = 0
					if(index == Store.currentSheetIndex){//發送消息者在當前頁面
    ...
    wsUpdateMsg: function(item) {
	    ...
	    if(type == "v"){ //單個單元格數據更新
	        ...
	        if(index == Store.currentSheetIndex){//更新數據為當前表格數據
	        ...
	        if(k == "borderInfo"){
				...
				// ⚠️ 上面的處理方式會導致部分配置項被遺漏，以致協同編輯的時候多視圖出現不一致的情況，調整處理的策略為直接替換配置項：
				// 可能的配置項為：
	        ...
	        else if(k == "filter_select"){ //篩選範圍
	            if(index == Store.currentSheetIndex){
	                createFilterOptions(value);
	            }
	        }
	        else if(k == "filter"){ //篩選保存
	            if(index == Store.currentSheetIndex){
	                createFilterOptions(file.filter_select, value);
	            }
	        }
	        else if(k == "luckysheet_conditionformat_save"){ //條件格式
	            if(index == Store.currentSheetIndex){
	                setTimeout(function () {
	                    luckysheetrefreshgrid();
	                }, 1);
	        ...
	        else if(k == "luckysheet_alternateformat_save"){ //交替顏色
	            if(index == Store.currentSheetIndex){
	                setTimeout(function () {
	                    luckysheetrefreshgrid();
	                }, 1);
	        ...
	        $("#luckysheet-cell-main").append('<div id="luckysheet-datavisual-selection-set-' + value.index + '" class="luckysheet-datavisual-selection-set"></div>');
					...
					// *添加sheet之後,要判斷是否需要顯示sheet滾動按鈕
					sheetmanage.locationSheet()
	    }
	    else if(type == "shc"){ //覆制sheet
	        ...
	        else if(op == "xy" || op == "wh" || op == "update"){ //移動 縮放 更新
    ...
    multipleRangeShow: function(id, name, r, c, value) {
	    ...
	    if($("#luckysheet-multipleRange-show-" + id).length > 0){
			...
			// 設定允許用戶名消失的定時器，10秒後用戶名可隱藏
			// 10秒之類，用戶操作界面不會隱藏用戶名；10秒之後如果用戶操作了界面，則隱藏用戶名，沒操作就不隱藏
			if(Store.cooperativeEdit.usernameTimeout['user' + id] != null){
				clearTimeout(Store.cooperativeEdit.usernameTimeout['user' + id])
			}
			Store.cooperativeEdit.usernameTimeout['user' + id] = setTimeout(()=>{
    ...
    sheetDeleSave: [], //共享編輯模式下 刪除的sheet保存下來，方便恢覆時取值
    submitInterval: 1000,
        ...
        _this.cachelocaldata(function(cahce_key, params){
            ...
            if(_this.updateUrl != ""){
                $.post(_this.updateUrl, { compress: iscommpress, gridKey: _this.gridKey, data: params }, function (data) {
                    ...
                    if(re.status){
                        $("#luckysheet_info_detail_update").html("最近存檔時間:"+ dayjs().format("M-D H:m:s"));
                        $("#luckysheet_info_detail_save").html("同步成功");
                        _this.clearcachelocaldata();
                    }
                    else{
                        $("#luckysheet_info_detail_save").html("<span style='color:#ff2121'>同步失敗</span>");
    ...
    imageRequest: function(){
        ...
        html2canvas($("#" + container).find(".luckysheet-grid-window").get(0), {
          onrendered: function(canvas) {
            ...
            //console.log(base64);
            //console.log("壓縮：", pako.gzip(base64, { to: "string" }));
            ...
            old.remove();
            //console.log("縮略圖", _this.imageRequestLast,base64);
            if(_this.updateImageUrl != ""){
                ...
                $.post(_this.updateImageUrl, { compress: false, gridKey: _this.gridKey, data:data1  }, function (data) {
                    ...
                    if(re.status){
                        ...
                        $("#luckysheet_info_detail_save").html("<span style='color:#ff2121'>網絡不穩定</span>");
                    }
                    _this.imageRequestLock =true;
                });
            }
