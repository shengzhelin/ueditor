...
function jfrefreshgrid(data, range, allParam, isRunExecFunction = true, isRefreshCanvas = true) {
    ...
    let RowlChange = allParam["RowlChange"];  //行高改變
    let cdformat = allParam["cdformat"];  //條件格式
    let dataVerification = allParam["dataVerification"];  //數據驗證
    let dynamicArray = allParam["dynamicArray"];  //動態數組
    ...
    if (Store.clearjfundo) {
        ...
        Store.jfredo.push({ 
            ...
            "range": range,
            "dataRange": [...file.luckysheet_select_save]// 保留操作時的選區
        });
    }
...
function jfrefreshgrid_adRC(data, cfg, ctrlType, ctrlValue, calc, filterObj, cf, af, freezen, dataVerification, hyperlink){
    ...
    //             // funcData存儲當前結果沒有用處，每次還是需要從calc公式鏈實時從當前數據中計算比較靠譜
...
function jfrefreshgrid_pastcut(source, target, RowlChange){
    ...
    if(RowlChange){
        ...
        for (let i = 0; i < rowHeight; i++) {
            ...
            Store.visibledatarow.push(Store.rh_height);//行的臨時長度分布
        }
        Store.rh_height += 80;
