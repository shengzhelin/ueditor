...
var make_ssf = function make_ssf(SSF) {
    ...
    */
    ...
    function format(fmt, v, o) {
        ...
        //new runze 增加萬 億 格式  
        //注："w":2萬2500  "w0":2萬2500  "w0.0":2萬2500.2  "w0.00":2萬2500.23......自定義精確度
        var reg = /^(w|W)((0?)|(0\.0+))$/;
        if(!!sfmt.match(reg)){
            if(isNaN(v)){
                return v;
            ...
            if( vlength> 4){
                if(vlength > 8){
                    var y =parseInt (v / 100000000);  //億
                    var w = parseInt(parseFloat(v).subtract(y*100000000) / 10000); //萬
                    var q = parseFloat(v).subtract(y*100000000 + w*10000); //千以後
                    if(acc != ""){
                        q = numeral(q).format(acc); //處理精確度
                    }
                    v = y + "億" + w + "萬" + q;
                }else{
                    var w = parseInt(v / 10000); //萬
                    var q = parseFloat(v).subtract(w*10000) //千以後
                    if(acc != ""){
                        q = numeral(q).format(acc); //處理精確度
                    }
                    v = w + "萬" + q;
                ...
                if(v.indexOf("億0萬0") != -1){
                    v = v.replace("0萬0","");
                }else if(v.indexOf("億0萬") != -1){
                    v = v.replace("0萬","");
                }else if(v.indexOf("萬0") != -1){
                    v = v.replace("萬0","萬");
                }

                //舍棄正則後顧斷言寫法，舊瀏覽器不識別（360 V9）
                if (v.indexOf("億") != -1 && v.indexOf("萬") == -1) { //1億/1億111 => 1億/1億0111
                    var afterYi = v.substring(v.indexOf("億") + 1);
                    if (afterYi.substring(0, 1) !== "." && afterYi != "") {
                        ...
                        v = v.substring(0, v.indexOf("億") + 1) + afterYi;
                    }
                } else if (v.indexOf("億") == -1 && v.indexOf("萬") != -1) { //3萬0011
                    var afterWan = v.substring(v.indexOf("萬") + 1);
                    if (afterWan.substring(0, 1) !== "." && afterWan != "") {
                        ...
                        v = v.substring(0, v.indexOf("萬") + 1) + afterWan;
                    }
                } else if (v.indexOf("億") != -1 && v.indexOf("萬") != -1) { //1億0053萬0611
                    var afterYi = v.substring(v.indexOf("億") + 1,v.indexOf("萬")),
                        afterWan = v.substring(v.indexOf("萬") + 1);
                    ...
                    v = v.substring(0, v.indexOf("億") + 1) + afterYi + v.substring(v.indexOf("萬"))
...
export function genarate(value) {//萬 單位格式增加！！！
    var ret = [];
    ...
    if (/^-?[0-9]{1,}[,][0-9]{3}(.[0-9]{1,2})?$/.test(value)) { // 表述金額的字符串，如：12,000.00 或者 -12,000.00
        m = value
