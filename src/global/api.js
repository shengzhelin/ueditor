...
/**
 * 獲取單元格的值
 * @param {Number} row 單元格所在行數；從0開始的整數，0表示第一行
 * @param {Number} column 單元格所在列數；從0開始的整數，0表示第一列
 * @param {Object} options 可選參數
 * @param {String} options.type 單元格的值類型，可以設置為原始值"v"或者顯示值"m"；默認值為'v',表示獲取單元格的實際值
 ...
 * 設置單元格的值
 *
 * 關鍵點：如果設置了公式，則需要更新公式鏈insertUpdateFunctionGroup，如果設置了不是公式，判斷之前是公式，則需要清除公式delFunctionGroup
 *
 * @param {Number} row 單元格所在行數；從0開始的整數，0表示第一行
 * @param {Number} column 單元格所在列數；從0開始的整數，0表示第一列
 * @param {Object | String | Number} value 要設置的值；可以為字符串或數字，或為符合Luckysheet單元格格式的對象
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Boolean} options.isRefresh 是否刷新界面；默認為`true`
 * @param {Function} options.success 操作結束的回調函數
 */
export function setCellValue(row, column, value, options = {}) {
    ...
    if (!method.createHookFunction("cellUpdateBefore", row, column, value, isRefresh)) {
        /* 如果cellUpdateBefore函數返回false 則不執行後續的更新 */
        return;
    }
 ...
 * @param {String} move 刪除後，右側還是下方的單元格移動。可選值為 'left'、'up'
 * @param {Number} row 單元格所在行數；從0開始的整數，0表示第一行
 * @param {Number} column 單元格所在列數；從0開始的整數，0表示第一列
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
...
/**
 * 設置某個單元格的屬性，如果要設置單元格的值或者同時設置多個單元格屬性，推薦使用setCellValue
 * @param {Number} row 單元格所在行數；從0開始的整數，0表示第一行
 * @param {Number} column 單元格所在列數；從0開始的整數，0表示第一列
 * @param {String} attr
 * @param {Number | String | Object} value 具體的設置值，一個屬性會對應多個值，參考 單元格屬性表的值示例，特殊情況：如果屬性類型attr是單元格格式ct，則設置值value應提供ct.fa，比如設置A1單元格的格式為百分比格式：luckysheet.setCellFormat(0, 0, "ct", "0.00%")
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數, callback參數為改變後的cell對象
 */
...
/**
 * 查找一個工作表中的指定內容，返回查找到的內容組成的單元格一位數組，數據格式同celldata
 * @param {String} content 要查找的內容 可以為正則表達式（不包含前後'/')
 * @param {Object} options 可選參數
 * @param {Boolean} options.isRegularExpression 是否正則表達式匹配；默認為 false. 注意：正則中的規則需要轉義，如\S需要寫成 \\S
 * @param {Boolean} options.isWholeWord 是否整詞匹配；默認為 false
 * @param {Boolean} options.isCaseSensitive 是否區分大小寫匹配；默認為 false
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {String} options.type 單元格屬性；默認值為m
 */
export function find(content, options = {}) {
    ...
    for (let i = 0; i < targetSheetData.length; i++) {
        ...
        for (let j = 0; j < rowArr.length; j++) {
            ...
            // 添加cell的row, column屬性
            // replace方法中的setCellValue中需要使用該屬性
            cell.row = i;
            cell.column = j;
...
/**
 * 查找一個工作表中的指定內容並替換成新的內容，返回替換後的內容組成的單元格一位數組，數據格式同celldata。
 * @param {String} content 要查找的內容
 * @param {String} replaceContent 要替換的內容
 * @param {Object} options 可選參數
 * @param {Boolean} options.isRegularExpression 是否正則表達式匹配；默認為 false
 * @param {Boolean} options.isWholeWord 是否整詞匹配；默認為 false
 * @param {Boolean} options.isCaseSensitive 是否區分大小寫匹配；默認為 false
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數, callback參數為替換後的cell集合
 */
...
/**
 * 凍結首行
 * 若設置凍結的sheet不是當前sheet頁，只設置參數不渲染
...
/**
 * 凍結行選區
 * @param {Object} range 行選區範圍的focus單元格的行列值構成的對象；格式為{ row_focus:0, column_focus:0 }
 * @param {Number | String} order 工作表索引
 */
...
/**
 * 凍結列選區
 * @param {Object} range 列選區範圍的focus單元格的行列值構成的對象；格式為{ row_focus:0, column_focus:0 }
 * @param {Number | String} order 工作表索引
 */
...
/**
 * 凍結行操作。特別注意，只有在isRange設置為true的時候，才需要設置setting中的range，且與一般的range格式不同。
 * @param {Boolean} isRange 是否凍結行到選區 true-凍結行到選區  false-凍結首行
 * @param {Object} options 可選參數
 * @param {Object} options.range isRange為true的時候設置，開啟凍結的單元格位置，格式為{ row_focus:0, column_focus:0 }，意為當前激活的單元格的行數和列數；默認從當前選區最後的一個選區中取得
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 凍結列操作。特別注意，只有在isRange設置為true的時候，才需要設置setting中的range，且與一般的range格式不同。
 * @param {Boolean} isRange 是否凍結列到選區 true-凍結列到選區  false-凍結首列
 * @param {Object} options 可選參數
 * @param {Object} options.range isRange為true的時候設置，開啟凍結的單元格位置，格式為{ row_focus:0, column_focus:0 }，意為當前激活的單元格的行數和列數；默認從當前選區最後的一個選區中取得
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
...
/**
 * 凍結行列操作。特別注意，只有在isRange設置為true的時候，才需要設置setting中的range，且與一般的range格式不同。
 * @param {Boolean} isRange 是否凍結行列到選區 true-凍結行列到選區  false-凍結首行列
 * @param {Object} options 可選參數
 * @param {Object} options.range isRange為true的時候設置，開啟凍結的單元格位置，格式為{ row_focus:0, column_focus:0 }，意為當前激活的單元格的行數和列數；默認從當前選區最後的一個選區中取得
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 ...
 * @param {Number} index 在第幾行插入空白行，從0開始
 * @param {Object} options 可選參數
 * @param {Number} options.number 插入的空白行數；默認為 1
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * @param {Number} row 在第幾行插入空白行，從0開始
 * @param {Object} options 可選參數
 * @param {Number} options.number 插入的空白行數；默認為 1
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 ...
 * @param {Number} column 在第幾列插入空白列，從0開始
 * @param {Object} options 可選參數
 * @param {Number} options.number 插入的空白列數；默認為 1
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 刪除指定的行或列。刪除行列之後，行列的序號並不會變化，下面的行（右側的列）會補充到上（左）面，注意觀察數據是否被正確刪除即可。
 * @param {String} type 刪除行或列 row-行  column-列
 * @param {Number} startIndex 要刪除的起始行或列
 * @param {Number} endIndex 要刪除的結束行或列
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 刪除指定的行。
 * @param {Number} rowStart 要刪除的起始行
 * @param {Number} rowEnd 要刪除的結束行
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 刪除指定的列。
 * @param {Number} columnStart 要刪除的起始列
 * @param {Number} columnEnd 要刪除的結束列
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 隱藏行或列
 * @param {String} type 隱藏行或列  row-隱藏行  column-隱藏列
 * @param {Number} startIndex 起始行或列
 * @param {Number} endIndex 結束行或列
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 顯示隱藏的行或列
 * @param {String} type 顯示行或列  row-顯示行  column-顯示列
 * @param {Number} startIndex 起始行或列
 * @param {Number} endIndex 結束行或列
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
...
/**
 * 設置指定行的高度。優先級最高，高於默認行高和用戶自定義行高。
 * @param {Object} rowInfo 行數和高度對應關系
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 設置指定列的寬度
 * @param {Object} columnInfo 行數和高度對應關系
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 獲取指定工作表指定行的高度，得到行號和高度對應關系的對象
 * @param {Array} rowInfo 行號下標組成的數組；行號下標從0開始；
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 獲取指定工作表指定列的寬度，得到列號和寬度對應關系的對象
 * @param {Array} columnInfo 行號下標組成的數組；行號下標從0開始；
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * @param   {Array}   range 可選參數，默認為當前選中區域
 * @returns {Array}   對象數組
 */
 ...
 * @param   {Array}   range 可選參數，默認為當前選中區域扁平化後的對象，結構形如[{r:0,c:0},{r:0,c:1}...]
 * @returns {Array}   對象數組
 */
...
/**
 * 返回對應當前選區的坐標字符串數組，可能存在多個選區。
 * 每個選區可能是單個單元格(如 A1)或多個單元格組成的矩形區域(如 D9:E12)
 * @returns {Array}
 */
 ...
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 */
 ...
 * @param {order} options.order 工作表下標
 */
export function getRangeHtml(options = {}) {
    ...
    if(cfg["borderInfo"] && cfg["borderInfo"].length > 0){ //邊框
...
/**
 * 覆制指定工作表指定單元格區域的數據，返回一維、二維或者自定義行列數的二維數組的數據。只有在dimensional設置為custom的時候，才需要設置setting中的row和column
 * @param {String} dimensional 數組維度。可選值為：oneDimensional-一維數組；twoDimensional-二維數組； custom-自定義行列數的二維數組
 * @param {Object} options 可選參數
 * @param {Number} options.row dimensional為custom的時候設置，多維數組的行數
 * @param {Number} options.column dimensional為custom的時候設置，多維數組的列數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 */
export function getRangeArray(dimensional, options = {}) {
    ...
    if(dimensional == 'oneDimensional'){//一維數組
        for(let r = r1; r <= r2; r++){
            for(let c = c1; c <= c2; c++){
                let cell = data[r][c];
...
/**
 * 覆制指定工作表指定單元格區域的數據，返回json格式的數據
 * @param {Boolean} isFirstRowTitle 是否首行為標題
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 */
...
/**
 *
 * @param {String} type 對角線還是對角線偏移 "normal"-對角線  "anti"-反對角線
"offset"-對角線偏移
 * @param {Object} options 可選參數
 * @param {Number} options.column type為offset的時候設置，對角偏移的列數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 */
 ...
 * @param {Array | Object | String} range 選區範圍
 * @param {Object} options 可選參數
 * @param {Boolean} options.show 是否顯示高亮選中效果；默認值為 `true`
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 將一個單元格數組數據賦值到指定的區域，數據格式同getRangeValue方法取到的數據。
 * @param {Array[Array]} data 要賦值的單元格二維數組數據，每個單元格的值，可以為字符串或數字，或為符合Luckysheet格式的對象
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Boolean} options.isRefresh 是否刷新界面；默認為true
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 設置指定範圍的單元格格式，一般用作處理格式，賦值操作推薦使用setRangeValue方法
 * @param {String} attr 要賦值的單元格二維數組數據，每個單元格的值，可以為字符串或數字，或為符合Luckysheet格式的對象
 * @param {Number | String | Object} value 具體的設置值
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 設置參數的目標選區範圍，支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 */
...
/**
 * 設置指定範圍的單元格格式，一般用作處理格式。支持多選區設置
 * @param {String} attr 要賦值的單元格二維數組數據，每個單元格的值，可以為字符串或數字，或為符合Luckysheet格式的對象
 * @param {Number | String | Object} value 具體的設置值
 * @param {Object} options 可選參數
 * @param {Array | Object | String} options.range 設置參數的目標選區範圍，支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 為指定索引的工作表，選定的範圍開啟或關閉篩選功能
 * @param {String} type 打開還是關閉篩選功能  open-打開篩選功能，返回當前篩選的範圍對象；close-關閉篩選功能，返回關閉前篩選的範圍對象
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Object} options.success 操作結束的回調函數
 */
...
/**
 * 為指定索引的工作表，選定的範圍開啟排序功能，返回選定範圍排序後的數據。
 * @param {String} type 排序類型 asc-升序 desc-降序
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
...
/**
 * 為指定索引的工作表，選定的範圍開啟多列自定義排序功能，返回選定範圍排序後的數據。
 * @param {Boolean} hasTitle 數據是否具有標題行
 * @param {Array} sort 列設置，設置需要排序的列索引和排序方式，格式如：[{ i:0,sort:'asc' },{ i:1,sort:'des' }]
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
...
/**
 *  為指定索引的工作表，選定的範圍開啟條件格式，根據設置的條件格式規則突出顯示部分單元格，返回開啟條件格式後的數據。
 * @param {String} conditionName 條件格式規則類型
 * @param {Object} conditionValue 可以設置條件單元格或者條件值
 * @param {Object} options 可選參數
 * @param {Object} options.format 顏色設置
 * @param {Array | Object | String} options.cellrange 選區範圍
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 為指定索引的工作表，選定的範圍開啟條件格式，返回開啟條件格式後的數據。
 * @param {String} type 條件格式規則類型
 * @param {Object} options 可選參數
 * @param {Array | String} options.format 顏色設置
 * @param {Array | Object | String} options.cellrange 選區範圍
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * @param {Number} itemIndex 條件格式規則索引
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * @param {Object} options 可選參數
 * @param {Array | Object | String} options.range 要清除的選區範圍
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 刪除指定工作表指定單元格區域，返回刪除掉的數據，同時，指定是右側單元格左移還是下方單元格上移
 * @param {String} move 刪除後，單元格左移/上移
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 要刪除的選區範圍
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * @param {String} type 計算方式
 * @param {Number} number 計算數值
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍,支持選區的格式為"A1:B2"、"sheetName!A1:B2"或者{row:[0,1],column:[0,1]}，只能為單個選區；默認為當前選區
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 新增一個sheet，返回新增的工作表對象
 * @param {Object} options 可選參數
 * @param {Object} options.sheetObject 新增的工作表的數據；默認值為空對象
 * @param {Number} options.order 新增的工作表索引；默認值為最後一個索引位置
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * 覆制指定下標的工作表到指定下標位置
 * @param {Object} options 可選參數
 * @param {Number} options.targetOrder 新覆制的工作表目標下標位置；默認值為當前工作表下標的下一個下標位置（遞增）
 * @param {Number} options.order 被覆制的工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 指定工作表向左邊或右邊移動一個位置，或者指定索引，返回指定的工作表對象
 * @param {String | Number} type 工作表移動方向或者移動的目標索引
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表索引；默認值為當前工作表索引
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * @param {Array} orderList 工作表順序，設置工作表的index和order來指定位置
 * @param {Object} options 可選參數
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 設置工作表縮放比例
 * @param {Number} zoom 工作表縮放比例，值範圍為0.1 ~ 4；
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * @param {Object} options 可選參數
 * @param {Number} options.order 需要顯示網格線的工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
 ...
 * 滾動當前工作表位置
 * @param {Object} options 可選參數
 * @param {Number} options.scrollLeft 橫向滾動值
 * @param {Number} options.scrollTop 縱向滾動值
 * @param {Number} options.targetRow 縱向滾動到指定的行號
 * @param {Number} options.targetColumn 橫向滾動到指定的列號
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 返回指定選區截圖後生成的base64格式的圖片
 * @param {Object} options 可選參數
 * @param {Object | String} options.range 選區範圍，只能為單個選區；默認為當前選區
 */
 ...
 * @param   {Object}    options         可選參數
 * @param   {Function}  options.success 操作結束的回調函數
 * @returns {String}    返回工作簿名稱，如果讀取失敗則返回空字符串並彈窗提示
 */
 ...
 * @param {String | Number} optionItem.value2 條件值2
 * @param {Boolean} optionItem.checked 選中狀態
 * @param {Boolean} optionItem.remote 自動遠程獲取選項
 * @param {Boolean} optionItem.prohibitInput 輸入數據無效時禁止輸入
 * @param {Boolean} optionItem.hintShow 選中單元格時顯示提示語
 * @param {String} optionItem.hintText 提示語文本
 * @param {Object} options 可選參數
 * @param {Array | Object | String} options.range 選區範圍；默認為當前選區
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 在指定的工作表中指定單元格位置插入圖片
 * @param {String} src 圖片src
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {Number} options.rowIndex 要插入圖片的單元格行下標；默認為0
 * @param {Number} options.colIndex 要插入圖片的單元格列下標；默認為0
 * @param {Function} options.success 操作結束的回調函數
 */
    ...
    if(file.index == Store.currentSheetIndex){
        ...
        if(visibledatarow.length === 0){
            ...
            for (let r = 0; r < rowheight; r++) {
                ...
                visibledatarow.push(rh_height); //行的臨時長度分布
            }
        }
        ...
        if(visibledatacolumn.length === 0){
            ...
            for (let c = 0; c < colwidth; c++) {
                ...
                visibledatacolumn.push(ch_width);//列的臨時長度分布
            }
        }
 ...
 * @param {Object} options 可選參數
 * @param {Number} options.order 工作表下標；默認值為當前工作表下標
 * @param {String | Array} options.idList 要刪除圖片的id集合，也可為字符串`"all"`，all為所有的字符串；默認為`"all"`
 * @param {Function} options.success 操作結束的回調函數
 */
...
/**
 * 導出的json字符串可以直接當作`luckysheet.create(options)`初始化工作簿時的參數`options`使用
 *
 */
...
/**
 * 傳入目標語言，切換到對應的語言界面
 * @param {String} lang 可選參數；暫支持`"zh"`、`"en"`、`"es"`；默認為`"zh"`；
 */
...
/**
 * 初始化分頁器
 * @param {Object} config 分頁器配置
 * @param {Number} config.pageIndex 當前的頁碼
 * @param {Number} config.pageSize 每頁顯示多少條數據
 * @param {Array} config.selectOption 選擇每頁的條數
 * @param {Number} config.total 總條數
 */
export function pagerInit (config) {
    ...
    $("#luckysheet-bottom-pager").sPage({
        page: config.pageIndex, //當前頁碼，必填
        total: config.total, //數據總條數，必填
        selectOption: config.selectOption, // 選擇每頁的行數，
        pageSize: config.pageSize, //每頁顯示多少條數據，默認10條
        showTotal: config.showTotal, // 是否顯示總數，默認關閉：false
        showSkip: config.showSkip, //是否顯示跳頁，默認關閉：false
        showPN: config.showPN, //是否顯示上下翻頁，默認開啟：true
        prevPage: config.prevPage || prevPage, //上翻頁文字描述，默認"上一頁"
        nextPage: config.nextPage || nextPage, //下翻頁文字描述，默認"下一頁"
        totalTxt: config.totalTxt || total + config.total, // 數據總條數文字描述，{total}為占位符，默認"總共：{total}"
...
/**
 * 更新sheet數據
 * @param {Array} data 工作簿配置，可以包含多個表
 * @param {Object} options 可選參數
 * @param {Function} options.success 操作結束的回調函數
 *
 */
