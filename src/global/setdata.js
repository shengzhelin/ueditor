...
function setcellvalue(r, c, d, v) {
    ...
    // 1.為null
    // 2.數據透視表的數據，flowdata的每個數據可能為字符串，結果就是cell == v == 一個字符串或者數字數據
    ...
    if(vupdateStr.substr(0, 1) == "'"){
        ...
        if(cell.f != null && isRealNum(vupdate) && !/^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i.test(vupdate)){
            ...
            if(mask === vupdate){ //若原來單元格格式 應用不了 要更新的值，則獲取更新值的 格式
...
//new runze 根據億萬格式autoFormatw和精確度accuracy 轉換成 w/w0/w0.00 or 0/0.0格式
