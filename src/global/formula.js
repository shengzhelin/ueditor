...
const luckysheetformula = {
    ...
    readCellDataToOneArray: function (rangeValue) {
        ...
        //適配excel的動態數組格式，{1，2，3，4，5}或者{{1，2，3}，{4，5，6}，{7，8，9}}
        if (getObjType(data) == "array") {
        ...
        //把二維數組轉換為一維數組，sparklines要求數據格式為一維數組
        //let dataformat = [];
        if (getObjType(data[0]) == "array") {
            for (let i = 0; i < data.length; i++) {
    ...
    //獲得某個單元格或區域的偏移一定距離後的單元格( Sheet1!B6:C8 格式)
    cellOffset: function (range, rows, cols, height, width) {// 參數：range or cell , rows,cols,height,width
    ...
    parseDatetoNum: function (date) { //函數中獲取到時間格式或者數字形式統一轉化為數字進行運算
        let _this = this;
        ...
        if (range.length == 1) { //一行
            for (let c = 0; c < range[0].length; c++) {
                if (range[0][c] != null && range[0][c].v) {
                    ...
                    //若單元格為null或為空，此處推入null（待考慮是否使用"null"）
                    rangeNow.push(null);
                }
            }
        }
    ...
    isWildcard: function (a, b) { //正則匹配通配符: * ? ~* ~?,a目標參數，b通配符
        let _this = this;
    ...
    isCompareOperator: function (str) { //判斷前一個或者兩個字符是否是比較運算符
        str = str.toString();
        let ope = ""; //存放比較運算符
        let num = ""; //截取比較運算符之後的數字用於實際比較
    ...
    acompareb: function (a, b) { //a 與 b比較，b可為含比較符，通配符
        let _this = this;
        let flag = false;
        ...
        else if (typeof (b) == "string") { //條件輸入字符串，如：">233"
            if (b.indexOf("*") != -1 || b.indexOf("?") != -1) { // 正則匹配：輸入通配符："黑*","白?",以及"白?黑*~*"
                //通配符函數
    ...
    compareParams: function (fp, sp, sym) {  //比較兩個字符串或者數字的大小，支持比較對象,暫不支持數組
        let flag = false;
    ...
    helpFunction: function ($editer, funcname, paramIndex) {
        ...
        //"a": "返回數據集的算術平均值",
        //"p": [{ "name": "數值1", "example": "A2:A100", "detail": "計算平均值時用到的第一個數值或範圍。", "require": "m", "repeat": "n", "type": "rangenumber" },
        //    { "name": "數值2", "example": "B2:B100", "detail": "計算平均值時用到的其他數值或範圍。", "require": "o", "repeat": "y", "type": "rangenumber" }
        //]
        let fht = "",
            ahf = "",
            fhcp = "";
        ...
        $("#luckysheet-formula-help-c .luckysheet-formula-help-title .luckysheet-arguments-parameter-holder").html(fht); //介紹
        $("#luckysheet-formula-help-c .luckysheet-arguments-help-formula .luckysheet-arguments-parameter-holder").html(ahf); //示例
        $("#luckysheet-formula-help-c .luckysheet-formula-help-content-param").html(fhcp); //參數
    ...
    updatecell: function (r, c, value, isRefresh = true) {
        ...
        if (!isCurInline) {
            ...
            if (getObjType(value) == "string" && value.slice(0, 1) == "=" && value.length > 1) {
                ...
                delete curv.m;//更新時間m處理 ， 會實際刪除單元格數據的參數（flowdata時已刪除）
                if (curv.f != null) { //如果原來是公式，而更新的數據不是公式，則把公式刪除
                    delete curv.f;
                    delete curv.spl; //刪除單元格的sparklines的配置串
                }
            }
        }
        ...
        let dynamicArrayItem = null;  //動態數組
        ...
        if ((d[r][c].tb == "2" && d[r][c].v != null) || isInlineStringCell(d[r][c])) {//自動換行
            let defaultrowlen = Store.defaultrowlen;

            let canvas = $("#luckysheetTableContent").get(0).getContext("2d");
            // offlinecanvas.textBaseline = 'top'; //textBaseline以top計算
        ...
        if (isRefresh) {
            ...
            // Store.luckysheetCellUpdate.length = 0; //clear array
            _this.execFunctionGlobalData = null; //銷毀
        }
        else {
            return {
                data: d,
    ...
    iscelldata: function (txt) { //判斷是否為單元格格式
        let val = txt.split("!"),
            rangetxt;
        ...
        let reg_cell = /^(([a-zA-Z]+)|([$][a-zA-Z]+))(([0-9]+)|([$][0-9]+))$/g; //增加正則判斷單元格為字母+數字的格式：如 A1:B3
        let reg_cellRange = /^(((([a-zA-Z]+)|([$][a-zA-Z]+))(([0-9]+)|([$][0-9]+)))|((([a-zA-Z]+)|([$][a-zA-Z]+))))$/g; //增加正則判斷單元格為字母+數字或字母的格式：如 A1:B3，A:A
    ...
    rangeSetValue: function (selected, obj) {
        ...
        if (_this.rangestart || _this.rangedrag_column_start || _this.rangedrag_row_start) {
            if ($("#luckysheet-search-formula-parm").is(":visible") || $("#luckysheet-search-formula-parm-select").is(":visible")) {
                ...
                //計算結果顯示
                let isVal = true; //參數不為空
                let parmValArr = []; //參數值集合
                let lvi = -1; //最後一個有值的參數索引
    ...
    functionHTML: function (txt) {
        ...
        while (i < funcstack.length) {
            ...
            //修正例如輸入公式='1-2'!A1時，只有2'!A1是luckysheet-formula-functionrange-cell色，'1-是黑色的問題。
            else if (s == "'" && matchConfig.dquote == 0) {
                str += "'";
        ...
        while (i < funcstack.length) {
            ...
            if (s == "(" && matchConfig.squote == 0 && matchConfig.dquote == 0 && matchConfig.braces == 0) {
                ...
                if (matchConfig.squote > 0) {
                    if (firstSQ == i - 1)//配對的單引號後第一個字符不能是單引號
                    {
                        return "";
                    }
                    //如果是''代表著輸出'
                    ...
                    else {//如果下一個字符不是'代表單引號結束
                        if (funcstack[i - 1] == "'") {//配對的單引號後最後一個字符不能是單引號
                            return "";
                        } else {
    ...
    execFunctionGroup: function (origin_r, origin_c, value, index, data, isForce = false) {
        ...
        if (value != null) {
            //此處setcellvalue 中this.execFunctionGroupData會保存想要更新的值，本函數結尾不要設為null,以備後續函數使用
        ...
        for (let i = 0; i < calcChains.length; i++) {
            ...
            if (isOffsetFunc) {
                ...
                //let formulaTextArray = calc_funcStr.split(/==|!=|<>|<=|>=|[,()=+-\/*%&^><]/g);//無法正確分割單引號或雙引號之間有==、!=、-等運算符的情況。導致如='1-2'!A1公式中表名1-2的A1單元格內容更新後，公式的值不更新的bug
                //解決='1-2'!A1+5會被calc_funcStr.split(/==|!=|<>|<=|>=|[,()=+-\/*%&^><]/g)分割成["","'1","2'!A1",5]的錯誤情況
                let point = 0;//指針
                let squote = -1;//雙引號
                let dquote = -1;//單引號
                let formulaTextArray = [];
                let sq_end_array = [];//保存了配對的單引號在formulaTextArray的index索引。
                ...
                for (let i = 0; i < calc_funcStr_length; i++) {
                    ...
                    if (char == "'" && dquote == -1) {
                        ...
                        {
                            //if (squote == i - 1)//配對的單引號後第一個字符不能是單引號
                            //{
                            //    ;//到此處說明公式錯誤
                            //}
                            //如果是''代表著輸出'
                            if (i < calc_funcStr_length - 1 && calc_funcStr.charAt(i + 1) == "'") {
                                ...
                                //if (calc_funcStr.charAt(i - 1) == "'") {//配對的單引號後最後一個字符不能是單引號
                                //    ;//到此處說明公式錯誤
                                    point = i + 1;
                ...
                //拼接所有配對單引號及之後一個單元格內容，例如["'1-2'","!A1"]拼接為["'1-2'!A1"]
                ...
                //至此=SUM('1-2'!A1:A2&"'1-2'!A2")由原來的["","SUM","'1","2'!A1:A2","",""'1","2'!A2""]更正為["","SUM","","'1-2'!A1:A2","","",""'1-2'!A2""]
    ...
    execfunction: function (txt, r, c, index, isrefresh, notInsertFunc) {
        ...
        try {
            ...
            if (typeof (result) == "string") {//把之前的非打印控制字符DEL替換回一個雙引號。
        ...
        //公式結果是對象，則表示只是選區。如果是單個單元格，則返回其值；如果是多個單元格，則返回 #VALUE!。
    ...
    //供function/functionImplementation.js的EVALUATE函數調用。
