...
function rowlenByRange(d, r1, r2, cfg) {
    ...
    canvas.textBaseline = 'top'; //textBaseline以top計算
...
function computeRowlenArr(rowHeight, cfg) {
    ...
    for (let i = 0; i < rowHeight; i++) {
        ...
        rowlenArr.push(rh_height);//行的臨時長度分布
    }
...
function getCellTextInfo(cell , ctx, option){
    ...
    let space_width = option.space_width, space_height = option.space_height; //寬高方向 間隙
    ...
    if(tr=="3"){//vertical text
        ...
        if(tb=="2" || isInline){//wrap
            ...
            let left = space_width + textHeight * Math.sin(rtPI)*isRotateUp; //默認為1，左對齊
            ...
            let top = (cellHeight - space_height)  - height + measureText.actualBoundingBoxAscent * Math.cos(rtPI) + textWidth * Math.sin(rtPI)*isRotateUp; //默認為2，下對齊
