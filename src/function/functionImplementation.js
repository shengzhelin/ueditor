...
const functionImplementation = {
    ...
    "ANIMAL_BY_IDCARD": function() {
        ...
        try {
            ...
            var list = new Array("豬", "鼠", "牛", "虎", "兔", "龍", "蛇", "馬", "羊", "猴", "雞", "狗");
        ...
        try {
            ...
            if(arguments[2]){
                ...
                if(rangeRow == sumRangeRow && rangeCol == sumRangeCol){
                    ...
                    //console.log("最終的目標範圍：",sumRangeData);
                }
    ...
    "SUBTOTAL": function() {
        ...
        try {
            //數字 1-11 或 101-111，用於指定要為分類匯總使用的函數
    ...
    "STDEVA": function() {
        ...
        try {
            ...
            //不是數值類型的值轉化成數字（true為1，false和文本為0）
            var dataArr_n = [];
    ...
    "PROB": function() {
        ...
        try {
            ...
            //data_x_range 和 data_prob_range 只取數值
    ...
    "HLOOKUP": function() {
        ...
        try {
            ...
            if (index > range.rowl) {
                return [formula.error.v, "索引超過了範圍的長度，" + range[0].length];
            }
            if (index < 1) {
                return [formula.error.v, "索引必須大於1"];
            }
    ...
    "LOOKUP": function() {
        ...
        try {
            ...
            if(typeof(searchkey) == "string"){ //字符串直接判斷是否相等
                ...
                for (var i = 0; i < range.length; i++) {
            ...
            }else if(isdatatype(searchkey) == "num"){ //數字判斷1.是否相等2.不等，去找接近值
                    var rangeNow = [];
                    ...
                    if(rangeNow.length != 0){
                        ...
                        if(index == (rangeNow.length -1)){
                            return [formula.error.na, "找不到對應參數"];
                        }else{
                            var mat = rangeNow[index + 1];
                            if(arguments[2]){
                                var i = range.indexOf(mat);//改成數組
                                result = range2[i];
                            }else{
                                result = mat;
                            }
        ...
        try {
            ...
            switch(unit){
                ...
                case "MD":case "md":
                    ...
                    break;
                case "YM":case "ym":
                    var startM = parseInt(startDate.format('M')); //注意字符串轉化為數字
        ...
        try {
            ...
            switch(info_type){
                ...
                case "prefix":
                    ...
                    if (sheetdata[row_index][col_index].ht == 0){//居中對齊
                        return "^";
                    }
                    else if (sheetdata[row_index][col_index].ht == 1){//左對齊
                        return "'";
                    }
                    else if (sheetdata[row_index][col_index].ht == 2){//右對齊
                        return '"';
                    }
                    else{
                        return "";
    ...
    "LINESPLINES": function() {
        ...
        try {
            ...
            //開始進行sparklines的詳細設置，寬和高為單元格的寬高。
            var sparksetting = {};
            ...
            //設置sparklines圖表的寬高，線圖的高會隨著粗細而超出單元格高度，所以減去一個量，設置offsetY或者offsetX為渲染偏移量，傳給luckysheetDrawMain使用。默認為0。=LINESPLINES(D9:E24,3,5)
            sparksetting["offsetY"] = lineWidth+1;
            sparksetting.height = height-(lineWidth+1);
            sparksetting.width = width;

            //定義sparklines的通用色彩設置函數，可以設置 色表【colorList】索引數值 或者 具體顏色值
            ...
            //設置輔助線，可以支持min、max、avg、median等幾個字符變量，或者具體的數值。
            if(!!normalValue){
    ...
    "SORT": function() {
        ...
        try {
            ...
            //表示所需排序方向的邏輯值；按行排序為FALSE（默認），按列排序為TRUE。
            var by_col = false;
            if(arguments.length == 4){
    ...
    "FILTER": function() {
        ...
        try {
            ...
            //如果包含數組中的所有值都為空(filter不返回任何值)，則返回的值
            var if_empty = "";
            if(arguments.length == 3){
    ...
    "UNIQUE": function() {
        ...
        try {
            ...
            //邏輯值，指示如何比較；按行 = FALSE 或省略；按列 = TRUE
            var by_col = false;
            if(arguments.length >= 2){
            ...
            //邏輯值，僅返回唯一值中出現一次 = TRUE；包括所有唯一值 = FALSE 或省略
