...
function renderCharts(chartLists, isDemo) {
    ...
    for (let i = 0; i < chartLists.length; i++) {
        ...
        //處理區域高亮框參數，當前頁中，只有當前的圖表的needRangShow為true,其他為false
        ...
        $t.mousedown(function (e) {  // move chart
                ...
                if (!chartInfo.chartparam.luckysheetCurrentChartMaxState) {
                    ...
                    //luckysheetCurrentChartMoveXy: [鼠標點相對chart框的距離X方向，鼠標點相對chart框的距離Y方向，chart框相對cell-main的距離X方向，chart框相對cell-main的距離Y方向，水平滾動條的位置，垂直滾動條的位置]
            ...
            .mousedown(function (e) {
                if (chartInfo.chartparam.luckysheetCurrentChartActive) {
                    chartInfo.chartparam.luckysheetCurrentChartResize = $(this).data("type"); //開始狀態resize
                    ...
                    //參數：x,y:鼠標位置，$t.width(), $t.height(): chart框寬高， position.left + scrollLeft, position.top + scrollTop ：chart框位置 ，scrollLeft, scrollTop：滾動條位置
    ...
    if (chart.rangeArray.length == 1) {
        ...
        var sheetIndex =
            luckysheetgetcellrange.sheetIndex == -1
                ? 0
                : luckysheetgetcellrange.sheetIndex; //sheetIndex為-1時，轉化為0
        ...
        if (
            ...
            //getcelldata有值，且不為空數組 && getcelldata.data為二維數組
...
/**
 * 設置圖表可拖動區域高亮效果，鼠標經過可拖動區域時鼠標顯示“十字”，不可拖動區域顯示箭頭
 * @param {JQuery} $container 圖表的容器DIV
 */
...
//設置某個圖表的高亮區域狀態為顯示,處理當前頁的所有圖表，只取一個圖表設置為顯示，其他隱藏，其他頁不管
...
function selectRangeBorderShow(chart_id) {
    ...
    //luckysheet取cell-main，後續擴展到其他的用戶自定義元素
    $('#luckysheet-cell-main')
    ...
    if (
        ...
        // TODO: 第一次創建圖表時候需要初始化數據選擇框 qkSelection
...
function hideChartSettingComponent(refresh) {
    if ($('.chartSetting').is(':visible')) {
        ...
        //隱藏設置界面
        $('.chartSetting').hide();
        //.luckysheet-modal-dialog-resize為圖表顯示框的縮放框，.luckysheet-modal-dialog-controll為顯示框右邊的控制按鈕
        $('#luckysheet-cell-main .luckysheet-modal-dialog-chart .luckysheet-modal-dialog-resize, #luckysheet-cell-main .luckysheet-modal-dialog-chart .luckysheet-modal-dialog-controll').hide()
...
// 隱藏其他sheet的圖表，顯示當前sheet的圖表 chartMix 切換sheet頁顯示隱藏圖表
function renderChartShow(index) {
    //傳入index，圖表顯示隱藏
    selectRangeBorderHide('true') //隱藏數據高亮區域，隨意傳入一個字符串，表示不處理chartSetting界面
        ...
        if (file.index == index) {
            ...
            chartLists.forEach((chart) => {
                ...
                if (chart.needRangeShow == true) {
                    //一個sheet頁只有一個圖表高亮顯示,//重要！因為在store了做了存儲，所以能在此處找到對應圖表設置顯示隱藏
                    //操作DOM當前圖表選擇區域高亮
