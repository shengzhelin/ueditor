...
func CreateTask(c *gin.Context) {
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("創建任務腳本失敗: %v", err.Error()))
		return
	}
...
func UpdateTask(c *gin.Context) {
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("更新腳本文件失敗，%v", err.Error()))
		return
	}
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("更改腳本文件名稱失敗，%v", err.Error()))
		return
	}
...
func DeleteTask(c *gin.Context) {
	...
	if fullName == "" {
		app.Error(c, -1, errors.New("參數不正確，請確定參數full_name是否傳遞"), "")
		return
	}

	err := os.Remove(fmt.Sprintf("%v/%v", viper.GetString("script.path"), fullName))
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("刪除文件失敗，%v", err.Error()))
		return
	}
...
func TaskDetails(c *gin.Context) {
	...
	if fileName == "" ||
		...
		strings.HasPrefix(fileName, "/") ||
		strings.HasPrefix(fileName, "\\") {
		app.Error(c, -1, errors.New("file_name參數不正確，請確認"), "")
		return
	}
