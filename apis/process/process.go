...
func ProcessList(c *gin.Context) {
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("查詢流程模版失敗，%v", err.Error()))
		return
	}
	app.OK(c, result, "查詢流程列表成功")
}
...
func CreateProcess(c *gin.Context) {
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("參數綁定失敗，%v", err.Error()))
		return
	}
	...
	if processCount > 0 {
		app.Error(c, -1, errors.New("流程名稱重覆"), "")
		return
	}
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("創建流程失敗，%v", err.Error()))
		return
	}

	app.OK(c, processValue, "流程創建成功")
}
...
func DeleteProcess(c *gin.Context) {
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("刪除流程失敗, %v", err.Error()))
		return
	}
	app.OK(c, "", "刪除流程成功")
}
...
func ProcessDetails(c *gin.Context) {
	...
	if err != nil {
		app.Error(c, -1, err, fmt.Sprintf("查詢流程詳情失敗, %v", err.Error()))
		return
	}

	app.OK(c, processValue, "查詢流程詳情成功")
}
...
func ClassifyProcessList(c *gin.Context) {
	...
	if processName == "" {
		...
		if err != nil {
			app.Error(c, -1, err, fmt.Sprintf("獲取分類列表失敗，%v", err.Error()))
			return
		}
	} else {
		err = orm.Eloquent.Model(&process2.Info{}).
		...
		if err != nil {
			app.Error(c, -1, err, fmt.Sprintf("獲取分類失敗，%v", err.Error()))
			return
		}
	...
	for _, item := range classifyList {
		...
		if err != nil {
			app.Error(c, -1, err, fmt.Sprintf("獲取流程失敗，%v", err.Error()))
			return
		}
	}
