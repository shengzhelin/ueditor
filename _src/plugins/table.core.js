...
(function() {
  ...
  /**
     * 獲取單元格或者單元格組的“對齊”狀態。 如果當前的檢測對象是一個單元格組， 只有在滿足所有單元格的 水平和豎直 對齊屬性都相同的
     * 條件時才會返回其狀態值，否則將返回null； 如果當前只檢測了一個單元格， 則直接返回當前單元格的對齊狀態；
     * @param table cell or table cells , 支持單個單元格dom對象 或者 單元格dom對象數組
  ...
  UETable.prototype = {
        ...
        for (var i = 0; i < this.rowsNum; i++) {
          ...
          for (var j = 0, tmpCell; (tmpCell = cells[j++]); ) {
            ...
            if (cell == tmpCell || end == tmpEnd) {
              //只獲取單一的單元格
              //todo 僅獲取單一單元格在特定情況下會造成returns為空，從而影響後續的拖拽實現，修正這個。需考慮性能
    ...
    /**
         ...
         */
    mergeRange: function() {
      //由於合併操作可以在任意時刻進行，所以無法通過鼠標位置等訊息實時生成range，只能通過緩存實例中的cellsRange對象來訪問
      var range = this.cellsRange,
      ...
      // 這段關於行表頭或者列表頭的特殊處理會導致表頭合併範圍錯誤
      // 為什麽有這段代碼的原因未明，暫且注釋掉，希望原作者看到後出面說明下
      // if (
      ...
      // 修改左上角Cell的rowSpan和colSpan，並調整寬度屬性設置
