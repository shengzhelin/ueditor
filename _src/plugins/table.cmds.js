...
(function () {
    ...
    UE.commands["mergeright"] = {
        queryCommandState: function (cmd) {
            ...
            if (rightColIndex >= ut.colsNum) return -1; // 如果處於最右邊則不能向右合並

            var rightCellInfo = ut.indexTable[cellInfo.rowIndex][rightColIndex],
                rightCell = table.rows[rightCellInfo.rowIndex].cells[rightCellInfo.cellIndex];
            if (!rightCell || cell.tagName != rightCell.tagName) return -1; // TH和TD不能相互合並

            // 當且僅當兩個Cell的開始列號和結束列號一致時能進行合並
    ...
    UE.commands["mergedown"] = {
        queryCommandState: function (cmd) {
            ...
            if (downRowIndex >= ut.rowsNum) return -1; // 如果處於最下邊則不能向下合並

            var downCellInfo = ut.indexTable[downRowIndex][cellInfo.colIndex],
                downCell = table.rows[downCellInfo.rowIndex].cells[downCellInfo.cellIndex];
            if (!downCell || cell.tagName != downCell.tagName) return -1; // TH和TD不能相互合並
    ...
    UE.commands['cellalignment'] = {
        ...
        /**
         * 查詢當前點擊的單元格的對齊狀態， 如果當前已經選擇了多個單元格， 則會返回所有單元格經過統一協調過後的狀態
    ...
    UE.commands["interlacetable"] = UE.commands["uninterlacetable"] = {
        queryCommandState: function (cmd) {
            ...
            if (cmd == "interlacetable") {
                //TODO 待定
                //是否需要待定，如果設置，則命令只能單次執行成功，但反射具備toggle效果；否則可以覆蓋前次命令，但反射將不存在toggle效果
