...
func Authenticator(c *gin.Context) (interface{}, error) {
	...
	if loginVal.LoginType == 1 {
		...
		if err != nil {
			return nil, errors.New(fmt.Sprintf("查詢用戶失敗，%v", err))
		}
		addUserInfo, err = ldap1.LdapFieldsMap(ldapUserInfo)
		if err != nil {
			return nil, fmt.Errorf("ldap映射本地字段失敗，%v", err.Error())
		}
		if authUserCount == 0 {
			...
			if err != nil {
				return nil, errors.New(fmt.Sprintf("查詢角色失敗，%v", err))
			}
			addUserInfo.RoleId = roleValue.RoleId // 綁定通用角色
			...
			if err != nil {
				return nil, errors.New(fmt.Sprintf("創建本地用戶失敗，%v", err))
			}
		}
	}
...
// @Accept  application/json
// @Product application/json
// @Success 200 {string} string "{"code": 200, "msg": "成功退出系統" }"
// @Router /logout [post]
// @Security
