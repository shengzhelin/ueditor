...
<table>
    ...
    <tr>
        ...
        <td>mergecell</td>
        <td>合並單元格</td>
        <td>主單元格{ r:0, c:0, rs: 2, cs:2 }，輔單元格{ r:0, c:0 }，<a href="#mergeCell">合並單元格案例</a></td>
        <td>Merge</td>
    </tr>
    <tr>
        <td>tr</td>
...
## 合並單元格<div id='mergeCell'></div>

設置一個合並單元格，需要處理兩個地方，一是單元格對象中設置`mc`屬性，二是在`config`中設置`merge`。

比如，將"A1:B2"單元格合並為一個單元格
...
- 第一步：先設置四個單元格的參數
    ...
    合並單元格的關鍵屬性是`mc`，主單元格是選區範圍中左上角的單元格，包含4個屬性 r/c/rs/cs ,分別表示 row/column/rowspan/columnspan，在這里，就表示從主單元格A1（row 0 column 0）開始到右邊2行下方2列的單元格進行合並，選區範圍中的其他單元格則只需要設置主單元格位置即可。
...
- 第二步：再設置`config.merge`
    ...
    ```
    對象中的key為 `r + '_' + c` 的拼接值，value同主單元格的`mc`設置: r:行數，c:列數，rs：合並的行數，cs:合並的列數

> merge 詳細參考：[表格數據config](/zh/guide/data.html#config)

## 含邊框單元格

單元格的邊框設置與合並單元格類似，需要在`config`中設置`borderInfo`，區別在於不需要設置單元格對象。
加入
