...
//    flag : true,
     ...
     *            測試啟動前處理步驟，默認為QUnit.stop();
     * @param op.onafterfinish
     *            測試完畢執行步驟，默認為QUnit.start()
     *
     */
...
//        return !record.join( '' );
    ...
    /*查看傳入的html是否與傳入的元素ele具有相同的style*/
...
//        ok(this.equalsNode(ele.innerHMTL,html),descript);
    ...
    checkElementPath:function (arr1, arr2, descript) {
        ...
        if (index != arr2.length)
            ...
            while (index > 0)
                equal(arr1[--index ], arr2[index ], descript + '---第' + index + '個元素' + arr1[index]);
        }
    },
    getBrowser:function () {
        var browser = "";
