...
test( 'Button', function () {
    ...
    equal( testButton.attributes['title'].nodeValue, 'testButton', '檢查Button的title ' );
} );

test( "工具欄國際化", function () {
...
test( "Dialog國際化", function () {
    ...
    editor.ready( function () {
        ...
        setTimeout(function(){
            ...
            ok(dialog.title===editor.getLang("labelMap.insertimage"),"dialog的標題是否一致");
            ok(dialog.closeDialog === editor.getLang("closeDialog"),"關閉dialog的title是否一致");
            ok(dialog.buttons[0].label === editor.getLang("ok") && dialog.buttons[1].label === editor.getLang("cancel"),"確認取消按鈕是否一致");
            setTimeout( function () {
                var dialogDoc = document.getElementById(dialog.id + "_iframe").contentWindow.document;
                var ids = editor.getLang("insertimage.static");
                for(var i in ids){
                    if(i==="imgSearchTxt"){
                        equal(dialogDoc.getElementById(i).value,ids[i].value,"圖片搜索文字是否一致")
                    }
                    if(i==="imgSearchBtn"){
                        equal(dialogDoc.getElementById(i).value,ids[i].value,"圖片搜索按鈕文字是否一致")
                    }
                }
                ua.click(document.getElementById(dialog.closeButton.id+"_body"));
                var newRange = editor.selection.getRange();
                ok( r.startContainer === newRange.startContainer, "dialog開閉前後選區是否一致" );
                start();
            }, 1500 )
        },100);
