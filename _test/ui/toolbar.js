...
test( 'toolbar顯示', function() {
    ...
    equal(toolbarShow.className,'edui-toolbar  '+theme,'檢查toolbar的顯示');
    equal(toolbarShow.childNodes.length ,toolbar.length,'檢查toolbar的顯示');
    for(var i=0;i<toolbar.length;i++){
        if(toolbar[i] =='|'){
            equal(toolbarShow.childNodes[i].className,'edui-box edui-separator '+theme,'檢查toolbar的顯示');
        }
        else{
            equal(toolbarShow.childNodes[i].className,'edui-box edui-button edui-for-' + toolbar[i] + theme,'檢查toolbar的顯示');
        }
    }
    ...
    setTimeout(function (){
        ...
        setTimeout(function (){
            ...
            toolbarUi.dispose();
            equal(document.getElementById('editor').childNodes.length,0,'成功銷毀');
            start();
        }, 400);
    }, 300);
    stop();
} );
test( 'toolbar點擊', function() {
    var toolbar =[ 'link','unlink'];
    baidu.editor.ui['link'] = function (cmd){
            return function (editor, title){
                var ui = new baidu.editor.ui.Button({
...
//    editor.render('editor');
    ...
    //要使用link,先要設置range和editor.body等內容
    ...
    ua.click(linkButton);
    setTimeout(function (){
        equal(document.body.lastChild.firstChild.firstChild.attributes['href'].nodeValue,'http://www.google.com/','點擊link按鈕後，生成連接');
        equal(document.body.lastChild.firstChild.firstChild.innerHTML,'hello','點擊link按鈕後，生成連接');
        toolbarUi.dispose();
