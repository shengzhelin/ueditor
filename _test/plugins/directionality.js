...
test( '非塊元素，閉合', function() {
    ...
    setTimeout(function () {
        ...
        editor.execCommand('directionality', "rtl");
        equal(ua.getChildHTML(db), "<p dir=\"rtl\"><strong><em>gggsiekes</em></strong></p>", "directionrtl");
        equal(editor.queryCommandValue('directionality'), "rtl", "查詢文字輸入方向為從右向左");

        editor.execCommand('directionality', "ltr");
        equal(ua.getChildHTML(db), "<p dir=\"ltr\"><strong><em>gggsiekes</em></strong></p>", "從左向右");
        equal(editor.queryCommandValue('directionality'), "ltr", "查詢文字輸入方向為從左向右");
        start();
    }, 20);
    stop();
} );
//4 range between blockelement and notblockelement
test( '選區包含塊和非塊元素', function() {
    var editor = te.obj[0],d = editor.document,range = te.obj[1],db = editor.body;
