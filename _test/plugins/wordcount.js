...
test('trace 1743 右鍵刪除後計算字數', function () {
    var editor = te.obj[0];
    var range = te.obj[1];
...
test('空格', function () {
    ...
    editor.setContent('           \ufeff\u200B\t\t    \n\n\t\n\b\t\n\b\u200B\t\t\n\n    ');
    if (ua.browser.ie)
        equal(editor.getContentLength(true), 23, '清空後編輯器中23個空格');
    else
        equal(editor.getContentLength(true), 22, '清空後編輯器中22個空格');
});
...
test(' trace 3744 超出最大', function () {
    ...
    editor.ready(function () {
        ...
        setTimeout(function () {
            ...
            equal(editor.getContentLength(true), 17, '僅統計字數')
