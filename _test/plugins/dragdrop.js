...
test( 'trace 3385：拖拽圖像不會把p切開', function () {
    var editor = te.obj[0];
    var range = te.obj[1];
    ...
    equal(body.childNodes.length,3,'img在兩個p之間');
    equal(body.firstChild.tagName.toLowerCase(),"p",'img在兩個p之間');
    equal(body.childNodes[1].tagName.toLowerCase(),"img",'img在兩個p之間');
    equal(body.childNodes[2].tagName.toLowerCase(),"p",'img在兩個p之間');
    ...
    setTimeout(function(){
        ...
        ua.dragend(body);
        setTimeout(function(){
            equal(body.childNodes.length,1,'拖拽圖像不會把p切開,刪除空span:img在p里面')
        equal(body.firstChild.tagName.toLowerCase(),'p','img在p里面');
        equal(body.firstChild.childNodes[1].tagName.toLowerCase(),'img','img在p里面');
            start();
        },300);
