...
test( 'trace1743 :content is null', function () {
    ...
    //TODO 現在必須先focus再selectall，trace1743
    editor.execCommand( 'selectAll' );
...
test( 'ctrl+a', function() {
    ...
    editor.setContent( '<p>全選的文本1</p><h1>全選的文本2</h1>' );
...
//            ua.checkResult( range, body, body, 0, 2, false, '查看全選後的range' );
//        else
           if(ua.browser.gecko||ua.browser.webkit){
               ua.checkResult( range, body, body, 0, 2, false, '查看全選後的range' );
           }else{
            ua.checkResult( range, body.firstChild.firstChild, body.lastChild.firstChild, 0, 6, false, '查看全選後的range' );
           }
        start();
    }, 150 );
    stop();
} );
test('contextmenu 右鍵全選', function () {
    var editor = te.obj[0];
    var range = te.obj[1];
    stop();
    ...
    equal(editor.selection.getRange().collapsed, true, '檢查選區--閉合');
    ua.click(menuBody.childNodes[0]);
    setTimeout(function () {
        equal(editor.selection.getRange().collapsed, false, '檢查選區--非閉合');
